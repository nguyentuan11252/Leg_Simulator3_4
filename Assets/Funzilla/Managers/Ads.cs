...
namespace Funzilla
{
	...
	internal class Ads : Singleton<Ads>
	{
		...
		private void LoadInterstitial(bool showImediately = false)
		{
		...
		internal static bool ShowInterstitial(string place, Action onFinished = null)
		{
			...
			{
				Instance._showingInterstitial = false;
				Instance.ShowReadyInterstitial(onFinished);
				return true;
			}
#if !AD_FAILED_EVENTS_ENABLED
			switch (Instance._interstitialState)
			{
				case InterstitialState.NotReady:
				case InterstitialState.LoadFailed:
				case InterstitialState.Available:
					Analytics.LogInterstitialFailedEvent(place);
					break;
				default:
