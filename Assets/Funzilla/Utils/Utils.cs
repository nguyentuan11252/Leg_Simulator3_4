...
namespace Funzilla
{
	internal class Utils : Singleton<Utils>
	{
		...
		internal void VibrateFlash()
		{
			if (Preference.Instance.VibrationOn && _vibrationCustomizable)
			{
				Vibrate(VibrationLevel.Flash);
			}
		}

		internal void VibrateLight()
		{
			if (Preference.Instance.VibrationOn && _vibrationCustomizable)
			{
